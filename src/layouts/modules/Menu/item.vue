...
<template>
  ...
  >
    <router-link v-slot="{ navigate }" custom :to="uniqueKey.at(-1) ?? ''">
      <HTooltip :enable="rootMenu.isMenuPopup && level === 0 && !subMenu" :text="typeof item.meta?.title === 'function' ? item.meta?.title() : item.meta?.title" placement="right" class="h-full w-full px-10px py-5px">
        <component
