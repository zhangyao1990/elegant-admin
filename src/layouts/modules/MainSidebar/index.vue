    ...
    <div v-if="settingsStore.settings.menu.menuMode === 'side' || (settingsStore.mode === 'mobile' && settingsStore.settings.menu.menuMode !== 'single')" class="main-sidebar-container">
      ...
      <div class="menu flex flex-col of-hidden transition-all">
        <template v-for="(item, index) in menuStore.allMenus" :key="index">
          <div
            class="relative mb-5px px-5px transition-all" :class="{
          ...
          >
            <div
              ...
              }" class="group menu-item-container mb-10px h-full w-full flex cursor-pointer items-center justify-between gap-1 rounded-8px py-5px text-[var(--g-main-sidebar-menu-color)] transition-all hover:(bg-[var(--g-main-sidebar-menu-hover-bg)] text-[var(--g-main-sidebar-menu-hover-color)]) px-2!" :title="typeof item.meta?.title === 'function' ? item.meta?.title() : item.meta?.title" @click="switchTo(index)"
            >
              <div

                class="w-full inline-flex flex-1 flex-col items-center justify-center gap-[2px]"
              >
