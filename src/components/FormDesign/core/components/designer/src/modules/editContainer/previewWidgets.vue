...
import type { PageManager, Revoke } from '../../../../../../utils'

import { useStore, useTimedQuery } from '../../../../../../hooks'
...
function scrollIntoView(selectorTop: number, selectorLeft: number) {
  ...
  if (Math.abs(newScrollTop - oldScrollTop) < 10 && Math.abs(newScrollLeft - oldScrollLeft) < 10) {
    return
  }
  oldScrollTop = newScrollTop
  oldScrollLeft = newScrollLeft
  if (newScrollTop > yMin && newScrollTop < yMax && newScrollLeft > xMin && newScrollLeft < xMax) {
    return
  }
...
// 初始化函数，传入一个指向 elegant 编辑范围的引用
function handleInit(elegantEditRangeRef) {
  kEditRange = elegantEditRangeRef
...
<template>
  ...
  <div
    ...
    ref="selectorRef"
    class="elegant-checked-widget pointer-events-none absolute z-20"
    :class="`${selectorPosition} ${selectorTransition ? 'transition-all' : ''}`"
  >
    <div ref="actionBoxRef" class="elegant-widget-action-box">
      <div class="elegant-widget-action-item whitespace-nowrap">
      ...
      <div v-if="isRemovableAndDraggable" class="flex items-center">
        <div
          title="复制"
          class="elegant-widget-action-item pointer-events-auto"
          @click="handleCopy"
        >
          <EIcon name="icon--elegant--copy-all-outline-rounded" />
        </div>
        <div
          title="删除"
          class="elegant-widget-action-item pointer-events-auto"
  ...
  <div
    ...
    ref="hoverWidgetRef"
    class="elegant-hover-widget pointer-events-none absolute z-998 transition-all"
  />
  <!-- 悬停效果 end  -->
</template>
